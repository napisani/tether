...
Treat `src/gtk/` as the reference for feature boundaries, terminology, navigation, and lifecycle behavior. Mirror it semantically, not line by line: keep the React code idiomatic and extract components where React benefits from doing so.
...
Before styling a web view, inspect both the corresponding `src/gtk/*_view.cpp` file and the shared GTK rules in `src/gtk/ui_util.cpp`. Preserve the same distinctions between selected and unselected rows, primary and secondary actions, muted and active status, setup guidance, errors, empty states, and route availability.
...
Before adding or changing a web feature, read the corresponding `src/gtk/*_view.cpp` and header. Account for:
...
When a feature changes in either client, update or add focused tests for the corresponding behavior in the other client when applicable.
